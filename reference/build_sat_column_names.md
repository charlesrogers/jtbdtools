...
Renames columns to the required `sat__job_step.objective` format. Runs
[`prep_data()`](https://charlesrogers.github.io/jtbdtools/reference/prep_data.md)
first, then prefixes each column with `sat__` and the job section.

## Usage

``` r
build_sat_column_names(df, job_section)
```
...
A data frame with columns renamed to `sat__job_section.original_name`
