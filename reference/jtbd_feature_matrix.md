...
Converts raw survey data into a numeric matrix suitable for PCA and
clustering. For each respondent and objective, calculates the
per-respondent opportunity score using the ODI formula:
`opp = imp + max(0, imp - sat)`.

## Usage

``` r
jtbd_feature_matrix(df)
```
...
A numeric matrix with respondents as rows and objectives as columns.
Column names are the objective identifiers (e.g.,
"researching.minimize_time_to_find_options").
